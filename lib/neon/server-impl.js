...
export async function renderPost(body, contentType) {
  try {
    let file, measurementScale = 1;
    if ((contentType || "").includes("application/x-www-form-urlencoded")) {
      // Large-file path: the browser uploaded the file to the backend and sent
      // us its URL (bypasses the serverless request-body limit). Fetch it here.
...
function rewriteAssets(markup) {
  ...
  return out.includes("</body>")
    ? out.replace("</body>", UPLOAD_BRIDGE_SCRIPT + "</body>")
    : out + UPLOAD_BRIDGE_SCRIPT;
}

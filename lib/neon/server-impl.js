...
// hiccup — the single backend Node process briefly blocked by a heavy job, so
// Cloudflare can't reach it in time — used to surface as a hard "Could not fetch the
// uploaded file (522)." even though the file was fine a second later. Retry a few
// times with backoff so transient blips self-heal. Zero added latency on the normal
// path; a real client error (404, etc.) still fails fast with its original status.

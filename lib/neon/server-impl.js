...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .result-info { min-width: 0; width: 100%; justify-self: stretch; margin-bottom: -1px; }
    ...
    .preview-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 20px; min-width: 0; width: 100%; max-width: 100%; height: 590px; padding-top: 0; margin-left: 0; align-self: stretch; }
    .preview-panel {
      min-width: 0;
    ...
    .preview-panel:has(.line-preview) { padding-left: 36px; padding-right: 36px; }
  ...
  <script>
    (() => {
      ...
      const unitPriceForBlock = (block) => {
        ...
        const hasUv = mountingSelect && mountingSelect.value === "6mm Clear Acrylic" && finishingSelect && finishingSelect.value === "UV Printing";
        const lineMeters = Number(priceTotal && priceTotal.dataset.lineMeters || 0);
        const sqft = Number(priceTotal && priceTotal.dataset.sqft || 0);
        const hasRgb = Boolean(block && block.querySelector('.color-option[data-color="RGB"].is-selected'));
        const neonCost = lineMeters * (hasRgb ? 80 : 25);
        const backingCost = sqft * 15;
        const uvCost = hasUv ? sqft * 7 : 0;
        const unitPrice = neonCost + backingCost + uvCost;
      ...
      const syncOrderTotals = (block) => {
        ...
        const baseUnitPrice = unitPriceForBlock(block);
        ...
        block.querySelectorAll(".agent-price-row").forEach((row) => {
          const agent = row.dataset.agent || "normal";
          const target = row.querySelector("strong");
          if (target) target.textContent = formatClientRM(agentPrices[agent] || total);
          const panel = row.closest(".checkout-panel");
          const currentAgent = (panel && panel.dataset.currentAgent) || "normal";
          row.classList.toggle("is-current", agent === currentAgent);
        });
        const priceTotal = block.querySelector(".total-price-card");
        if (priceTotal) {
          priceTotal.dataset.currentBaseUnitPrice = baseUnitPrice.toFixed(2);
          priceTotal.dataset.currentMultiplier = multiplier.toFixed(2);
          priceTotal.dataset.currentUnitPrice = unitPrice.toFixed(2);
          priceTotal.dataset.currentQuantity = String(quantity);
          priceTotal.dataset.currentTotal = total.toFixed(2);
          const target = priceTotal.querySelector("strong");
          const note = priceTotal.querySelector("small");
          if (target) target.textContent = formatClientRM(unitPrice);
        ...
        if (addCartButton) {
          ...
          try {
            ...
            window.parent.postMessage({
              type: "sign-cart-add",
              item: { label: "Neon Sign", href: "/neon-line", price: price, meta: metaParts.join(", ") },
...
function renderArtboard(artboard, pricing) {
  ...
  const title = artboard.name || `Artboard ${artboard.page || ""}`.trim();
  const dimensionPreview = artboard.dimension_preview_url ? `<div class="preview-panel"><div class="panel-label">Dimension Preview</div><div class="preview dimension-preview"><img src="${artboard.dimension_preview_url}" alt="Dimension preview"></div><div class="preview-total">Content size: <strong>${contentSize}</strong></div></div>` : "";
  const linePreview = artboard.line_preview_url ? `<div class="preview-panel"><div class="panel-label">Line Preview</div><div class="preview line-preview"><img src="${artboard.line_preview_url}" alt="Line preview"></div><div class="line-total">Line total: <strong>${artboard.total_length_m_neon.toFixed(2)} m</strong></div></div>` : "";
  ...
  return `
    <div class="result">
      ...
      <div class="preview-workspace">
        ...
        ${renderDesignSummary(contentSize, lineTotal, artboard.by_color, content, artboard.total_length_m_neon, pricing)}
      </div>
...
function renderCollectDateOptions() {
  return `<div class="collect-date-panel"><span>Collect Date</span><div class="collect-date-grid"><button type="button" class="collect-date-option is-selected" data-multiplier="1" aria-label="4 working days"><span class="collect-date-zoom" aria-label="Preview 4 working days">&#128269;</span><img src="/assets/collect-date-4-working-days.png" alt="4 working days"><span>4 working days</span></button><button type="button" class="collect-date-option" data-multiplier="1.2" aria-label="3 working days"><span class="collect-date-zoom" aria-label="Preview 3 working days">&#128269;</span><img src="/assets/collect-date-3-working-days.png" alt="3 working days"><span>3 working days</span></button><button type="button" class="collect-date-option" data-multiplier="1.35" aria-label="2 working days"><span class="collect-date-zoom" aria-label="Preview 2 working days">&#128269;</span><img src="/assets/collect-date-2-working-days.png" alt="2 working days"><span>2 working days</span></button><button type="button" class="collect-date-option" data-multiplier="1.7" aria-label="Next working days"><span class="collect-date-zoom" aria-label="Preview next working days">&#128269;</span><img src="/assets/collect-date-next-working-days.png" alt="Next working days"><span>Next working days</span></button></div></div>`;
}

function renderCheckoutOptions() {
  return `<div class="checkout-panel" data-current-agent="normal"><span>Order</span><div class="set-price-row agent-price-row is-current" data-agent="normal"><span>Normal Agent Price</span><strong>RM 0.00</strong></div><div class="set-price-row agent-price-row" data-agent="gold"><span>Gold Agent Price</span><strong>RM 0.00</strong></div><div class="set-price-row agent-price-row" data-agent="platinum"><span>Platinum Agent Price</span><strong>RM 0.00</strong></div><div class="quantity-row"><span>Quantity</span><div class="quantity-control"><button type="button" class="qty-button" data-qty="minus" aria-label="Decrease quantity">-</button><input class="quantity-input" type="number" min="1" step="1" value="1" aria-label="Quantity"><button type="button" class="qty-button" data-qty="plus" aria-label="Increase quantity">+</button></div></div><label class="terms-row"><input type="checkbox" class="terms-checkbox"><span>I agree to the <button type="button" class="terms-link">terms and conditions</button></span></label><button type="button" class="add-cart-button" disabled>Add to cart</button></div>`;
}
...
export async function renderPost(body, contentType) {
  try {
    ...
    const result = await analyzeUpload(upload.file, measurementScale);
    return rewriteAssets(html(result, null, await readNeonPricing()));
  } catch (error) {

...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Power Supply modal (shown from Add-to-Cart). */
  ...
  <script>
    (() => {
      ...
      const syncOrderTotals = (block) => {
        ...
        const powerQty = Math.max(0, parseInt((block.querySelector(".power-supply-input") || {}).value || "0", 10) || 0);
        const powerCost = powerQty * 60;
        const tierTotal = tierUnit.map((u) => u * quantity + powerCost);
        const agentPrices = {};
      ...
      // is a content-sized iframe, so plain fixed-centering lands in the middle of
      // the tall content — off-screen).
      const centerModalCard = (dialog) => {
        if (!dialog) return;
        const h = dialog.offsetHeight || 220;
        let cy;
        try { cy = window.parent.innerHeight / 2 - window.frameElement.getBoundingClientRect().top; }
        catch (_e) { cy = window.innerHeight / 2; }
        dialog.style.position = "fixed";
        dialog.style.left = "50%";
        dialog.style.transform = "translateX(-50%)";
        dialog.style.top = Math.max(16, cy - h / 2) + "px";
      };
      // Lock/unlock the PARENT page scroll so a popup holds focus.
      ...
      window.addEventListener("change", (event) => {
        ...
        if (event.target && event.target.matches(".terms-checkbox")) {
          ...
          if (button) {
            ...
            button.disabled = !event.target.checked || !uploaded;
            button.classList.remove("is-added");
            button.textContent = uploaded ? "Add to cart" : "Upload artwork first";
          }
        }
      ...
      window.addEventListener("click", (event) => {
        ...
        if (psStep) {
          ...
          if (inp) {
            const cur = Math.max(0, parseInt(inp.value || "0", 10) || 0);
        ...
        if (psYes || psNo) {
          ...
          const inp = modal && modal.querySelector(".ps-modal-input");
          const qty = psYes && inp ? Math.max(0, parseInt(inp.value || "0", 10) || 0) : 0;
          if (pendingPowerCard) {
            const store = pendingPowerCard.querySelector(".power-supply-input");
            if (store) store.value = qty;
            syncOrderTotals(pendingPowerCard);
          }
        ...
        if (addCartButton) {
          ...
          // Power Supply: ask on Add-to-Cart (neon signs are always lit, so it
          // always applies) before adding to the same order.
          if (addCartButton.dataset.psOk !== "1") {
            showPowerSupplyModal(addCartButton, reqBlock);
            return;
          }
          ...
          try {
            ...
            // Capture the total at EVERY member tier [Agent, Silver, Gold, Diamond]
            // so the cart can re-price automatically if the member upgrades or
            // downgrades after adding — instead of freezing the add-time price.
            const readTier = function(a){ var r = block && block.querySelector('.agent-price-row[data-agent="' + a + '"] strong'); return r ? (parseFloat(r.textContent.replace("RM", "").replace(/,/g, "").trim()) || 0) : price; };
            ...
            // Capture the craftsmanship the customer chose — mounting/acrylic base,
            // UV printing and the neon colours — so the admin order, Job Order and
            // Invoice list every spec (not just the size). Parts are "Label: Value"
            // joined by " . " so checkout's metaToOptions splits them into rows.
            const mountingSel = block && block.querySelector(".mounting-base-select");
            const finishingSel = block && block.querySelector(".finishing-select");
            const mounting = (mountingSel && mountingSel.value) || "6mm Clear Acrylic";
            const finishing = mounting === "6mm Clear Acrylic" && finishingSel ? (finishingSel.value || "None") : "None";
            const neonColors = block
              ? Array.from(block.querySelectorAll(".color-picker .color-option.is-selected")).map((o) => o.dataset.color).filter(Boolean).join(" / ")
              : "";
            const metaParts = [];
            if (baseSizeEl && baseSizeEl.textContent) metaParts.push("Size: " + baseSizeEl.textContent.trim() + " (W x H)");
            if (lineEl && lineEl.textContent) metaParts.push("Line: " + lineEl.textContent.trim());
            if (neonColors) metaParts.push("Neon Color: " + neonColors);
            metaParts.push("Mounting Base: " + mounting);
            metaParts.push("Surface Finishing: " + finishing);
            var psInputEl = block && block.querySelector(".power-supply-input");
            ...
            metaParts.push("Qty: " + qty + " set");
            window.parent.postMessage({
              type: "sign-cart-add",
              item: { label: "Neon Sign", href: "/neon-line", price: price, tierPrices: tierPrices, meta: metaParts.join(" · "), status: pending ? "Pending Confirmation" : undefined },
...
function renderCheckoutOptions() {
  return `<div class="checkout-panel" data-current-agent="agent"><input type="hidden" class="power-supply-input" value="0"><span>Order</span><div class="set-price-row agent-price-row is-current" data-agent="agent"><span>Agent Price</span><strong>RM 0.00</strong></div><div class="set-price-row agent-price-row" data-agent="silver"><span>Silver Agent Price</span><strong>RM 0.00</strong></div><div class="set-price-row agent-price-row" data-agent="gold"><span>Gold Agent Price</span><strong>RM 0.00</strong></div><div class="set-price-row agent-price-row" data-agent="diamond"><span>Diamond Agent Price</span><strong>RM 0.00</strong></div><div class="order-pending-note"></div><div class="quantity-row"><span>Quantity</span><div class="quantity-control"><button type="button" class="qty-button" data-qty="minus" aria-label="Decrease quantity">-</button><input class="quantity-input" type="number" min="1" step="1" value="1" aria-label="Quantity"><button type="button" class="qty-button" data-qty="plus" aria-label="Increase quantity">+</button></div></div><label class="terms-row"><input type="checkbox" class="terms-checkbox"><span>I agree to the <button type="button" class="terms-link">terms and conditions</button></span></label><button type="button" class="add-cart-button" disabled>Add to cart</button></div>`;
}
...
// keep-alive connection to Cloudflare — the request hangs with no response) even though
// the file is fine (a curl of the same URL is instant); a quick retry gets a fresh
// connection. Fail FAST on a stall (short per-attempt timeout) and try several times;
// the timer guards the body read too. A real client error (404 etc.) fails fast with
// its status. Returns { status, buf } — buf is null on failure.
...
const UPLOAD_BRIDGE_SCRIPT = `<script>(function(){
  ...
  function bind(){
    ...
    form.addEventListener("submit",function(e){
      ...
      // Upload the file ONCE to the backend, then analyze it by URL. Previously a
      // normal-size file was uploaded TWICE at the same time — once to the analyze
      // route and once to save the artwork — so the two transfers split the
      // uplink and the progress bar crawled at half speed. The single upload IS
      // the artwork; the analyze route fetches it server-side (fast).

  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .result {
      position: relative;
      display: grid;
      grid-template-columns: 502px 370px 430px;
      gap: 18px;
      align-items: start;
    ...
    .design-body { display: grid; grid-template-columns: 502px 370px 430px; column-gap: 18px; row-gap: 8px; align-items: start; justify-content: space-between; }
    .result-info { grid-column: 3; grid-row: 1 / 3; min-width: 0; width: 430px; justify-self: end; margin-bottom: -1px; }
    ...
    .led-color-chip {
      ...
      height: 100%;
      border-radius: 6px;
      background: linear-gradient(135deg, #15243a, #07101d);
      box-shadow: inset 0 0 12px rgba(255,255,255,0.06);
    }
    .led-color-chip.rgb-chip {
      background: conic-gradient(from 0deg, #ff0040, #ff8a00, #ffe600, #29ff5a, #00e0ff, #6a5cff, #ff00d4, #ff0040);
      box-shadow: inset 0 0 14px rgba(255,255,255,0.18), 0 0 10px rgba(255,255,255,0.12);
      animation: rgbChipSpin 4s linear infinite;
    }
    @keyframes rgbChipSpin { to { filter: hue-rotate(360deg); } }
    .led-color-option[data-led-color="RGB"] .led-color-chip { display: block; }
    ...
    .led-color-option[data-led-color="None"] {
      padding: 8px 4px;
      align-content: center;
    }
    .led-color-option[data-led-color="None"] .led-color-chip { display: none; }
    .led-color-option strong {
      position: relative;
      z-index: 1;
      font-size: 11px;
      line-height: 1;
    ...
    .dimension-zoom-button { top: 10px; right: 10px; }
    ...
    .preview-grid { grid-column: 1; grid-row: 1; display: grid; grid-template-columns: 1fr; gap: 12px; min-width: 0; width: 100%; max-width: 100%; padding-top: 4px; align-self: start; }
    .preview-panel {
      min-width: 0;
      height: 390px;
      margin-top: -3px;
    ...
    @media (max-width: 1120px) {
      ...
      .preview-grid, .letter-dimensions, .craft-3d-preview, .result-info { grid-column: auto; grid-row: auto; }
    ...
    (() => {
      ...
      const syncBaseFinishing = () => {
        document.querySelectorAll(".mounting-base-select").forEach((select) => {
          ...
          if (!finishing) return;
          const showFinishing = select.value === "3mm White Acrylic";
      ...
      const syncCraft3D = (scope) => {
        ...
        setCraftVar("--face", isBlack ? "#06080d" : (ledColor === "3000K" ? "#fff1c2" : ledColor === "4200K" ? "#fff8ea" : ledColor === "10000K" ? "#e6f6ff" : "#fffdf5"));
        ...
        panel.querySelectorAll("[data-three-led-label]").forEach((item) => {
          item.textContent = ledColor === "3000K" ? "3000K Warm White" : ledColor === "4200K" ? "4200K Neutral White" : ledColor === "10000K" ? "10000K Cool White" : "";
        });
        panel.querySelectorAll("[data-craft-depth-label]").forEach((item) => item.textContent = boxSize + " side wall");
        panel.querySelectorAll("[data-craft-surface]").forEach((item) => item.textContent = surface + (finishing !== "None" ? " + " + finishing : ""));
        panel.querySelectorAll("[data-craft-size]").forEach((item) => item.textContent = getSelectedSideFinishing(scope) + " / " + boxSize + " thickness" + sideFinishingSummary(scope));
        panel.querySelectorAll("[data-craft-color]").forEach((item) => item.textContent = boxColor);
        ...
        panel.querySelectorAll("[data-craft-back]").forEach((item) => item.textContent = scope.querySelector(".base-finish-material-select")?.value || "10mm PVC Foam Board");
      ...
      const syncOrderPanel = (panel) => {
        ...
        scope?.querySelectorAll(".letter-dimension-item").forEach((item) => {
          ...
          const isLogo = /^Logo\b/i.test(label);
          ...
          const itemPrice = Math.max(width, height) * (baseRate + ledRate + uvRate);
        ...
        prices.forEach((price) => {
          ...
          price.textContent = "RM " + (unit * quantity * multiplier).toLocaleString("en-MY", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        });
      };
      ...
      window.addEventListener("click", (event) => {
        ...
        if (zoomButton) {
          openSimpleImageModal(zoomButton.dataset.zoomSrc, zoomButton.dataset.zoomAlt);
          event.preventDefault();
          event.stopPropagation();
          return;
        }
        ...
        if (dateOption) {
          ...
          syncCraft3D(dateOption.closest(".result, .design-card"));
          return;
        }
      ...
      window.addEventListener("pointerdown", (event) => {
        ...
        const frame = event.target.closest(".dimension-record-hitbox");
        if (!frame) return;
        const scope = frame.closest(".result, .design-body");
        const panel = scope && scope.querySelector(".letter-dimensions");
        const item = itemByRecordId(panel, frame.dataset.recordId);
        if (!item || !item.classList.contains("is-selected")) return;
        draggingFrame = { item, panel, x: event.clientX, y: event.clientY };
  ...
  <script type="module">
    ...
    const ledMaterialSettings = (ledColor, isBlack) => {
      ...
      const settings = {
        ...
        "4200K": { color: 0xfff8e8, emissive: 0xfff1d0, intensity: 3.8, coreOpacity: 0.18, bloomOpacity: 0.42, lightIntensity: 0.82, bloomStrength: 0.7, bloomRadius: 0.36, bloomThreshold: 0.32 },
    ...
    const applyLedFaceMaterial = (state, ledColor, isBlack, isUvPrinting = false) => {
      ...
      if (face.uniforms?.ledPower) {
        const enabled = !isBlack && ledColor !== "None";
        face.uniforms.baseColor.value.setHex(isBlack ? 0x000000 : led.color);
        face.uniforms.glowColor.value.setHex(led.emissive || 0x000000);
        ...
        face.color.setHex(led.color);
        face.emissive.setHex(led.emissive);
        face.emissiveIntensity = led.intensity;
        face.transparent = !isBlack && ledColor !== "None";
        face.opacity = led.opacity || 1;
      ...
      if (state.faceGlow) {
        ...
        const baseGlowScale = ledColor === "3000K" ? 1.018 : ledColor === "4200K" ? 1.017 : ledColor === "10000K" ? 1.018 : 1.002;
      ...
      if (state.ledFaceLight) {
        state.ledFaceLight.visible = !isBlack && ledColor !== "None";
      ...
      if (state.bloomPass) {
        ...
        const faceBloom = !isBlack ? led.bloomStrength * glowControls.strengthMultiplier : 0;
      ...
      state.rgbAnim = ledColor === "RGB" && !isUvPrinting && (!isBlack || translucentSide);
    ...
    const animateRgbFace = (state) => {
      ...
      if (face && face.uniforms?.glowColor) {
        face.uniforms.baseColor.value.copy(base);
    ...
    const createFrontFaceGlowMesh = (geometry, ledColor, isBlack) => {
      ...
      const led = ledMaterialSettings(ledColor, isBlack);
      const enabled = !isBlack && ledColor !== "None";
      const glowScale = ledColor === "3000K" ? 1.018 : ledColor === "4200K" ? 1.017 : ledColor === "10000K" ? 1.018 : 1.002;
      ...
      glowGroup.traverse((object) => object.layers.set(1));
      return glowGroup;
    };
    ...
    const createMaterialSet = (isBlack, ledColor, sideConfig, isUvPrinting = false) => {
      ...
      const createShellMaterial = (colorName, factor, bevel = false) => {
        ...
        const color = isWhite ? new THREE.Color(filamentSideColor(colorName)) : tonedColor(filamentSideColor(colorName), factor);
        const emissive = isTranslucent ? new THREE.Color(sideGlowHex(colorName, ledColor)) : new THREE.Color(0x000000);
        const shell = new THREE.MeshPhysicalMaterial({
          color,
          emissive,
          emissiveIntensity: isTranslucent ? (bevel ? 1.15 : 0.92) : 0,
          metalness: isWhite ? 0 : bevel ? 0.02 : 0.03,
          roughness: isWhite ? 0.82 : isTranslucent ? (bevel ? 0.24 : 0.32) : bevel ? 0.42 : 0.58,
          ...
          envMapIntensity: isWhite ? 0.06 : isTranslucent ? 0.9 : bevel ? 0.72 : 0.42,
    ...
    const buildLetterMesh = async (state, panel) => {
      ...
      const mesh = new THREE.Mesh(geometry, createMaterialSet(isBlack, ledColor, sideConfig, surface === "3mm White Acrylic" && finishing === "UV Printing"));
    ...
    const createCraftScene = async (stage) => {
      ...
      ledFaceLight.position.set(0, 0.24, 2.95);
...
function renderLedColorOptions() {
  const ledColors = [
    ["None", ""],
    ["3000K", "led-color-3000k.jpg"],
    ["4200K", "led-color-4500k.jpg"],
    ["10000K", "led-color-10000k.jpg"],
    ["RGB", ""],
  ];
  const options = ledColors.map(([color, file], index) => {
    const media = file
      ? `<img src="/assets/${file}" alt="${color} LED color">`
      : color === "RGB"
        ? '<span class="led-color-chip rgb-chip"></span>'
        : '<span class="led-color-chip"></span>';
    return `<button type="button" class="led-color-option${index === 0 ? " is-selected" : ""}" data-led-color="${color}" aria-label="${color} LED color">${media}<strong>${color}</strong></button>`;
  }).join("");
...
function renderCollectDateOptions() {
  const dates = [
    ["4 working days", "collect-date-4-working-days.png"],
    ["3 working days", "collect-date-3-working-days.png"],
    ["2 working days", "collect-date-2-working-days.png"],
    ["Next working days", "collect-date-next-working-days.png"],
  ];
  const options = dates.map(([label, file], index) => {
    const src = `/assets/${file}`;
    return `<button type="button" class="collect-date-option${index === 0 ? " is-selected" : ""}" aria-label="${escapeHtml(label)}"><img src="${src}" alt="${escapeHtml(label)}"><span>${escapeHtml(label)}</span><span class="image-zoom-button" data-zoom-src="${src}" data-zoom-alt="${escapeHtml(label)}" aria-label="View ${escapeHtml(label)} larger"></span></button>`;
  }).join("");
...
function renderOrderPanel() {
  return `<div class="order-panel" data-unit-price="634.77">
    <span>Order</span>
    <div class="order-total-row is-active-agent"><span>Normal Agent Price</span><strong class="order-price" data-agent-tier="normal" data-agent-multiplier="1">RM 0.00</strong></div>
    ...
    <div class="order-total-row"><span>Platinum Agent Price</span><strong class="order-price" data-agent-tier="platinum" data-agent-multiplier="0.8">RM 0.00</strong></div>
...
async function analyzeUpload(file, measurementScale = 1, wordingText = "") {
  ...
  // data URLs) at the top level and on every artboard / design.
  const bytes = file.data instanceof Uint8Array ? file.data : new Uint8Array(file.data);
  const result = await analyzeBoxup(bytes, file.filename, measurementScale);
...
export async function renderPost(body, contentType, appRoute = "/3d-box-up/app") {
  try {
    ...
    const measurementScale = upload.fields.measurementScale === "10" ? 10 : 1;
    const result = await analyzeUpload(upload.file, measurementScale, upload.fields.wordingText || "");
    return rewriteAssets(html(result), appRoute);
  } catch (error) {
...
<script>(function(){
  var MAP={"3000K":"/3d-box-up/render/3000k.png","4200K":"/3d-box-up/render/4200k.png","10000K":"/3d-box-up/render/10000k.png","None":"/3d-box-up/render/none.png"};
...
function rewriteAssets(markup, appRoute = "/3d-box-up/app") {
  const signboardFlag = appRoute.indexOf("3d-signboard") !== -1
    ...
    : "";
  return markup
    .split("</body>").join(signboardFlag + LED_HD_FEATURE + "</body>")

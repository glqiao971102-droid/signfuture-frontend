...
function renderLetterDimensions(dimensions) {
  ...
  const ledTotalRow = showLed && totalLedM > 0
    ...
    : "";
  return `<div class="letter-dimensions"><div class="letter-dimensions-title"><span>Letter / Logo records (<span class="record-count">${dimensions.length}</span>)</span><span class="record-action-buttons"><button type="button" class="group-selected-button">Select with Group</button><button type="button" class="special-finishing-button" disabled>Special Finishing</button><button type="button" class="delete-selected-button" disabled>Delete</button><button type="button" class="undo-button" disabled>Restore</button></span></div><div class="letter-dimension-head"><span>Item</span><span>Width x Height / Price</span></div><div class="letter-dimension-list">${items}</div>${ledTotalRow}</div>`;
}

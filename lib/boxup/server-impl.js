...
<head>
  ...
  <script>
    (() => {
      ...
      // 3D Printer Frontlit price list: RM per inch by letter-size bracket
      // (in inches), unlit vs lit. Each entry is [upperInch, ratePerInch].
      const frontlitLetterRate = (size, hasLed) => {
        const unlit = [[6, 7], [12, 6], [30, 4.5], [36, 6], [42, 8], [47, 10]];
        const lit = [[6, 14], [12, 12], [30, 11], [36, 12], [42, 16], [47, 20]];
        const table = hasLed ? lit : unlit;
        for (const [upTo, rate] of table) {
          if (size <= upTo) return rate;
        }
        return table[table.length - 1][1]; // above the top bracket
      };
      const syncOrderPanel = (panel) => {
        ...
        // (Read the lit mode off <body> — this runs in a plain script, so the
        // module-scoped LIT_MODE const is not in scope here.)
        ...
        const is3dPrinter = frontlit && !!boxUpColorSelect &&
          Array.from(boxUpColorSelect.options || []).some((o) => /2K Spray/i.test(o.textContent || o.value || ""));
        // 2K Spray adds RM1 per inch to each letter's rate, and carries a
        // RM200 minimum for the whole piece.
        ...
        scope?.querySelectorAll(".letter-dimension-item").forEach((item) => {
          ...
          let itemPrice;
          if (is3dPrinter) {
            itemPrice = size * (frontlitLetterRate(size, hasLed) + (spray ? 1 : 0));
          } else {
        ...
        // 2K spray carries a RM200 minimum per piece.
        if (spray && unit > 0 && unit < 200) unit = 200;
...
export async function renderPost(body, contentType, appRoute = "/3d-box-up/app") {
  try {
    ...
    const result = await analyzeUpload(upload.file, measurementScale, upload.fields.wordingText || "", upload.spec || null);

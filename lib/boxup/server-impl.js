...
// The uploaded artwork lives on the backend behind Cloudflare. A momentary origin
// hiccup — the single backend Node process briefly blocked by a heavy job (nesting /
// UV-layout / pdfium render), so Cloudflare can't reach it in time — used to surface
// as a hard "Could not fetch the uploaded file (522)." even though the file was fine
// and fetchable a second later. Retry a few times with backoff so transient blips
// self-heal. Zero added latency on the normal path (first attempt succeeds); a real
// client error (404 missing, etc.) still fails fast with its original status.
async function fetchUploadWithRetry(url, attempts = 3) {
  let res = null, err = null;
  for (let i = 0; i < attempts; i++) {
    if (i > 0) await new Promise((r) => setTimeout(r, i === 1 ? 700 : 1800));
    try {
  ...
  if (res) return res;
  throw err || new Error("Could not fetch the uploaded file.");
}
...
export async function renderPost(body, contentType, appRoute = "/3d-box-up/app") {
  try {
    ...
    if ((contentType || "").includes("application/x-www-form-urlencoded")) {
      ...
      const res = await fetchUploadWithRetry(fileUrl);

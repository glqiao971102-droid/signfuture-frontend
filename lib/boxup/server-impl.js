  ...
  <script>
    (() => {
      ...
      const syncOrderPanel = (panel) => {
        ...
        const is3dPrinter = frontlit && !/EG Box Up/i.test(document.body.dataset.boxupName || "") && !!boxUpColorSelect &&
        ...
        const isStainless = /Stainless Steel Box Up/i.test(boxupName);
        ...
        const isEg = /EG Box Up/i.test(boxupName);
...
const UPLOAD_BRIDGE_SCRIPT = `<script>(function(){
  ...
  function saveArtwork(file, API){
    // Save the artwork to the backend IN PARALLEL with analysis (doesn't block
    // it). Stash the saved ref in sessionStorage (survives the result page's
    // document.write) so the parent (ProductFrame) can lock it into the cart.
    try{
      var fd=new FormData(); fd.append("file",file);
      fetch(API+"/api/v1/uploads/artwork",{method:"POST",body:fd})
        .then(function(r){ return r.ok?r.json():null; })
        .then(function(j){ if(j&&j.url){ var u=/^https?:/.test(j.url)?j.url:API+j.url; try{ sessionStorage.setItem("__SF_ARTWORK", JSON.stringify({url:u,name:file.name})); }catch(_){} } })
        .catch(function(){});
    }catch(_){}
  }
  function bind(){
    ...
    form.addEventListener("submit",function(e){
      ...
      if(file.size<=LIMIT){
        // Normal size: analyze DIRECTLY (fast — no backend round-trip) and save
        // the artwork in parallel.
        saveArtwork(file, API);
        var x=new XMLHttpRequest(); x.open("POST",form.getAttribute("action"));
        x.upload.onprogress=function(ev){ if(ev.lengthComputable) set(Math.round(ev.loaded/ev.total*100)); };
        x.upload.onload=function(){ analyzing(); };
        x.onload=function(){ if(x.status>=200&&x.status<300) render(x.responseText); else fail("Something went wrong ("+x.status+"). Please try again."); };
        x.onerror=function(){ fail("Network error. Please try again."); };
        x.send(new FormData(form));
      } else {
        // Large file: must go via the backend (serverless body limit). That one
        // upload is also the artwork (embedded server-side via __SF_ARTWORK).
        var fd=new FormData(); fd.append("file",file); var up=new XMLHttpRequest(); up.open("POST",API+"/api/v1/uploads/artwork");
...
function rewriteAssets(markup, appRoute = "/3d-box-up/app", artwork) {
  ...
  const artScript =
    artwork && artwork.url
      ...
      : "";
  return markup
    .split("</body>").join(signboardFlag + LED_HD_FEATURE + artScript + UPLOAD_BRIDGE_SCRIPT + "</body>")

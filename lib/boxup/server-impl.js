...
export async function renderPost(body, contentType, appRoute = "/3d-box-up/app") {
  try {
    let file, fields = {}, spec = null;
    if ((contentType || "").includes("application/x-www-form-urlencoded")) {
      // Large-file path: browser uploaded to the backend, sent us the URL.
      ...
      file = { filename: fileUrl.split("/").pop() || "artwork", data: Buffer.from(await res.arrayBuffer()) };
    } else {
    ...
    const result = await analyzeUpload(file, measurementScale, fields.wordingText || "", spec || null);
...
const UPLOAD_BRIDGE_SCRIPT = `<script>(function(){
  function bind(){
    var fi = document.querySelector('form[enctype="multipart/form-data"] input[type="file"][name="file"]');
    var form = fi && fi.form;
    if(!form || form.__sfBound){ return; } form.__sfBound = true;
    var API = (location.hostname==="localhost"||location.hostname==="127.0.0.1") ? "http://localhost:3333" : "https://api.signfuturegroup.com";
    form.addEventListener("submit", function(e){
      var f = form.querySelector('input[type="file"][name="file"]');
      if(!f || !f.files || !f.files[0]){ return; }
      e.preventDefault();
      var btn = form.querySelector('button[type="submit"]');
      var label = btn && btn.textContent; if(btn){ btn.disabled = true; btn.textContent = "Uploading…"; }
      ...
      fetch(API + "/api/v1/uploads/artwork", { method:"POST", body: fd })
        ...
        .then(function(j){
          ...
          var params = new URLSearchParams(); params.set("fileUrl", url);
          new FormData(form).forEach(function(v,k){ if(k!=="file" && typeof v==="string"){ params.set(k,v); } });
          return fetch(form.getAttribute("action"), { method:"POST", headers:{"Content-Type":"application/x-www-form-urlencoded"}, body: params });
        })
        .then(function(r){ return r.text(); })
        .then(function(h){ document.open(); document.write(h); document.close(); })
        .catch(function(err){ if(btn){ btn.disabled=false; btn.textContent=label; } alert("Could not process the file: " + (err && err.message || err)); });
    });
  }
...
function rewriteAssets(markup, appRoute = "/3d-box-up/app") {
  const signboardFlag = appRoute.indexOf("3d-signboard") !== -1
    ...
    : "";
  return markup
    .split("</body>").join(signboardFlag + LED_HD_FEATURE + UPLOAD_BRIDGE_SCRIPT + "</body>")

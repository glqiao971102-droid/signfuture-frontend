...
const PRODUCT_NAME = "Hand Flag";
const PRODUCT_HREF = "/catalog/hand-flag";

// Options from the old Sign Future wind-flag product page.
const FINISHING = ["Printing with Pole", "Pole with Cap Only"];
const MATERIAL = "Flag Fabric 80gsm";
...
const SIZE = [
  { label: '12"(W) x 6"(H) (40cm pole)', w: 304.8, h: 152.4 },
];
...
const PRICE: Record<string, number[]> = {
  "Printing with Pole": [6.27, 3.1, 3.1, 3.1],
  "Pole with Cap Only": [3, 1.5, 1.5, 1.5],
};
...
export default function HandFlagProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  // A bare pole is this product's no-print case, so it drives the same
  // hide-the-print-options logic the stand products use.
  const standOnly = finishing === "Pole with Cap Only";
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/hand-flag-hero.png", meta: `Finishing: ${finishing}${standOnly ? "" : ` · Material: ${MATERIAL}`} · Size: ${size} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/hand-flag-hero.png"
        ...
        <div className="xprod-summary-col">
          <aside className="xprod-summary">
            ...
            <div className="xprod-summary-list">
              <div className="xprod-sline is-wide"><span data-icon="▤">Material</span><strong>{standOnly ? "Pole only" : MATERIAL}</strong></div>

...
const PRODUCT_NAME = "S Shape Display";
const PRODUCT_HREF = "/catalog/s-shape-display";
...
const SIZE = [
  { label: '81cm (W) x 276cm (H)', w: 810, h: 2760 },
];
const SEWING = ["Sewing With Silicon"];
...
const PRICE: Record<string, number[]> = {
  "Printing with Stand": [993.3, 496.6, 496.6, 496.6],
  "Printing Only": [610.5, 305.2, 305.2, 305.2],
  "Stand Only": [382.8, 191.4, 191.4, 191.4],
};
...
export default function SShapeDisplayProduct() {
  const { user } = useAuth();
  const { add } = useCart();

  const [finishing, setFinishing] = useState(FINISHING[0]);
  const [sewing, setSewing] = useState(SEWING[0]);
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/s-shape-display-hero.png", meta: `Material: ${MATERIAL} · Finishing: ${finishing} · Size: ${size} · Sewing: ${sewing} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/s-shape-display-hero.png"
      ...
      <div className="xprod-grid">
        ...
        <section className="xprod-panel">
          ...
          <div className="xprod-field" data-icon="✎">
            ...
            >
              {SEWING.map((o) => <option key={o}>{o}</option>)}
            </select>
          </div>
        ...
        <div className="xprod-summary-col">
          <aside className="xprod-summary">
            ...
            <div className="xprod-summary-list">
              ...
              <div className="xprod-sline is-wide"><span data-icon="✎">Sewing Type</span><strong>{sewing}</strong></div>

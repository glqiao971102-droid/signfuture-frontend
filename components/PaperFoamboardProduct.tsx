...
import { PAPER_PRICE } from "@/lib/paperFoamboardPrices";

const PRODUCT_NAME = "Paper Foamboard";
const PRODUCT_HREF = "/catalog/paper-foamboard";

// Paper Foamboard keeps its own banner; everything else mirrors PVC Foamboard.
...
// Paper Foamboard is stocked in 5mm only.
const THICKNESS = ["5mm"];
...
export default function PaperFoamboardProduct() {
  ...
  // Per-size price from the paper foamboard price table (RM per piece), chosen by
  // finishing (Diecut Only / Diecut + UV Printing) x size, then x qty. Thickness is 5mm only.
  const finishKey = finishing.includes("UV Printing") ? "uv" : "die";
  const sizeKey = `${sizeOpt.h}x${sizeOpt.w}`;
  const baseTiers = PAPER_PRICE[finishKey]?.[sizeKey] ?? [0, 0, 0, 0];
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: HERO_IMAGE, deliverable: true, tierPrices: tierTotals, spec: { pricer: "lookup", key: "paper-foamboard", path: [finishKey, sizeKey], collect, qty }, meta: `Material: Paper Foamboard · Finishing: ${finishing} · Thickness: ${thickness} · Size: ${sizeLabel} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src={HERO_IMAGE}
          alt="Custom Paper Foamboard — CNC Cut, premium quality"
      ...
      <div className="xprod-grid">
        ...
        <section className="xprod-panel">
          <div className="xprod-head"><span className="xprod-head-icon" aria-hidden="true">▤</span><h2 className="xprod-stitle" data-kicker="Options">Paper Foamboard Order</h2></div>
          ...
          <div className="xprod-field" data-icon="▦">
            ...
            <select value={thickness} onChange={(e) => setThickness(e.target.value)} disabled={THICKNESS.length === 1}>
        ...
        <div className="xprod-summary-col">
          <aside className="xprod-summary">
            ...
            <div className="xprod-summary-list">
              <div className="xprod-sline"><span data-icon="▤">Material</span><strong>Paper Foamboard</strong></div>

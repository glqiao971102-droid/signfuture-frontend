...
const PRODUCT_NAME = "Tower (Curve)";
const PRODUCT_HREF = "/catalog/tower-curve";
...
const SIZE = [
  { label: '90cm (W) x 191cm (H)', w: 900, h: 1910 },
];
...
export default function TowerCurveProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/tower-curve-hero.png", meta: `Finishing: ${finishing} · Material: ${MATERIAL} · Size: ${size} · Sewing Type: ${sewing} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/tower-curve-hero.png"

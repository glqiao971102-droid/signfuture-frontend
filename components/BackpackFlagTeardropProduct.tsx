...
const PRODUCT_NAME = "Backpack Flag (Teardrop)";
const PRODUCT_HREF = "/catalog/backpack-flag-teardrop";
...
const SIZE = [
  { label: '450mm(W) x 1000mm(H)', w: 450, h: 1000 },
];
...
const PRICE: Record<string, number[]> = {
  "Printing with Stand": [270.6, 135.3, 135.3, 135.3],
  "Printing Only": [102.3, 51.15, 51.15, 51.15],
  "Stand Only": [168.3, 84.2, 84.2, 84.2],
};
...
export default function BackpackFlagTeardropProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/backpack-flag-teardrop-hero.png", meta: `Finishing: ${finishing} · Material: ${standOnly ? "Stand only" : MATERIAL} · Size: ${size} · Qty: ${qty}${standOnly ? "" : ` · ${collectOpt.label}`}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/backpack-flag-teardrop-hero.png"

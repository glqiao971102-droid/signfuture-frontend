...
const PRODUCT_NAME = "Backpack Flag (Rectangle)";
const PRODUCT_HREF = "/catalog/backpack-flag-rectangle";

// Options from the old Sign Future wind-flag product page.
const FINISHING = ["Printing with Stand", "Printing Only", "Stand Only"];
const MATERIAL = "Mesh fabric 110gsm";
...
const SIZE = [
  // Sold as a set of two panels, so the quoted area covers both.
  { label: '400mm(W) x 1000mm(H) = 2pcs = 1Set', w: 400, h: 1000, pcs: 2 },
];
const MM2_PER_SQFT = 92903.04;
// pcs covers sizes sold as a multi-panel set - the quote must bill every panel.
const areaOf = (s: { h: number; w: number; pcs?: number }) =>
  ((s.h * s.w) / MM2_PER_SQFT) * (s.pcs ?? 1);
...
const PRICE: Record<string, number[]> = {
  "Printing with Stand": [326.7, 163.4, 163.4, 163.4],
  "Printing Only": [141.9, 71, 71, 71],
  "Stand Only": [184.8, 92.4, 92.4, 92.4],
};
...
export default function BackpackFlagRectangleProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/backpack-flag-rectangle-hero.png", meta: `Finishing: ${finishing} · Material: ${standOnly ? "Stand only" : MATERIAL} · Size: ${size} · Qty: ${qty}${standOnly ? "" : ` · ${collectOpt.label}`}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/backpack-flag-rectangle-hero.png"

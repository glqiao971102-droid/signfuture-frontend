...
const PRODUCT_NAME = "Tower (Oblique)";
const PRODUCT_HREF = "/catalog/tower-oblique";
...
const SIZE = [
  { label: '84cm (W) x 209cm (H)', w: 840, h: 2090 },
];
...
const PRICE: Record<string, number[]> = {
  "Printing with Stand": [828.3, 414.2, 414.2, 414.2],
  "Printing Only": [445.5, 222.75, 222.75, 222.75],
  "Stand Only": [382.2, 191.4, 191.4, 191.4],
};
...
export default function TowerObliqueProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/tower-oblique-hero.png", meta: `Finishing: ${finishing} · Material: ${MATERIAL} · Size: ${size} · Sewing Type: ${sewing} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/tower-oblique-hero.png"

...
const SIZE = ['24"(W) x 72"(H) — 4 Eyelet'];
...
export default function XStandProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  // Per-tier price by Finishing x Print Tech x Lamination (per piece).
  let tierUnit: number[];
  if (standOnly) {
    ...
    tierUnit =
      tech === "UV Ink 1200dpi" ? fp.uv : fp.eco[lam === "No Laminate" ? "none" : "lam"];
  }
  const tierTotals = tierUnit.map((v) => Math.max(0, v * qty * collectOpt.mult));
  // Customer pays their own tagged tier (Agent for guests). Cheaper tiers are
  // only unlocked by topping up — never selectable here.
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/x-stand-hero.png"
          alt="Custom X Standee — premium quality"
            ...
            <div className="xprod-order" style={{ border: 0, background: "transparent", boxShadow: "none", margin: 0, padding: 0 }}>
              ...
              {added && (
                <div className="xprod-added">
                  <span>✓ X Stand added to cart — {money(agents[0].price)}</span>

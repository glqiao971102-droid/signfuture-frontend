...
// Bevel Edge Frame is made from clear cast acrylic only.
const MATERIAL = [
...
export default function AcrylicBevelEdgeFrameProduct() {
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/acrylic-bevel-edge-frame-hero.png", meta: `Material: ${material} · Thickness: ${thickness} · Size: ${sizeOpt.label} · Finishing: ${finishing} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/acrylic-bevel-edge-frame-hero.png"
          alt="Custom Acrylic Bevel Edge Frame (with Boltnut) — CNC Cut, premium quality"
      ...
      <div className="xprod-grid">
        ...
        <section className="xprod-panel">
          <div className="xprod-head"><span className="xprod-head-icon" aria-hidden="true">▤</span><h2 className="xprod-stitle" data-kicker="Options">Acrylic Bevel Edge Frame (with Boltnut) Order</h2></div>

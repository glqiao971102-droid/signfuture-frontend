...
const MATERIAL = "Black Back Tarpaulin 440gsm";

// Eyelet count follows the panel: the taller 2-side board needs four more.
const SIDE = [
  { label: "(W)195cm x (H)95cm (1 Side)",  w: 195, h: 95,  finish: "Welded edge with 18 eyelets" },
  { label: "(W)195cm x (H)200cm (2 Side)", w: 195, h: 200, finish: "Welded edge with 26 eyelets" },
];
...
const PRICE: Record<string, Record<string, Tier4>> = {
  "Printing with Stand": { "(W)195cm x (H)95cm (1 Side)": [339.4, 282.8, 282.8, 282.8], "(W)195cm x (H)200cm (2 Side)": [423.7, 353.1, 353.1, 353.1] },
  "Printing Only": { "(W)195cm x (H)95cm (1 Side)": [81.9, 68.3, 68.3, 68.3], "(W)195cm x (H)200cm (2 Side)": [166.3, 138.6, 138.6, 138.6] },
};
const STAND_ONLY_PRICE: Tier4 = [257.4, 214.5, 214.5, 214.5];
...
export default function AluminiumAboardStand100x200Product() {
  const { user } = useAuth();
  const { add } = useCart();

  const [finishing, setFinishing] = useState(FINISHING[0]);
  const [tech, setTech] = useState(PRINT_TECH[0]);
  const [side, setSide] = useState(SIDE[0].label);
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: "Aluminium Aboard Stand 100cm x 200cm", href: "/catalog/aluminium-aboard-stand-100x200", price: total, image: "/products/aluminium-aboard-stand-100x200-hero.png", meta: `Finishing: ${finishing}${standOnly ? "" : ` · Material: ${MATERIAL} · Printing: ${tech} · Side: ${side} · Material Finishing: ${sideOpt.finish}`} · Qty: ${qty}${standOnly ? "" : ` · ${collectOpt.label}`}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/aluminium-aboard-stand-100x200-hero.png"
          alt="Custom Aluminium Aboard Stand 100cm x 200cm — premium quality"
      ...
      <div className="xprod-grid">
        ...
        <section className="xprod-panel">
          <div className="xprod-head">
            ...
            <h2 className="xprod-stitle" data-kicker="Options">Aluminium Aboard Stand 100cm x 200cm Order</h2>
          </div>
          ...
          {!standOnly && (
            <>
              ...
              <div className="xprod-field" data-icon="⤢">
                ...
                <select value={side} onChange={(e) => setSide(e.target.value)} disabled={SIDE.length === 1}>
                  {SIDE.map((o) => <option key={o.label}>{o.label}</option>)}
                </select>
              </div>

              {/* Eyelet count is fixed by the chosen panel, so this reads back
                  the side's own finish rather than offering a free choice. */}
              <div className="xprod-field" data-icon="✎">
                <label>Material Finishing</label>
                <select disabled value={sideOpt.finish}>
                  <option>{sideOpt.finish}</option>
                </select>
              </div>
        ...
        <div className="xprod-summary-col">
          <aside className="xprod-summary">
            ...
            <div className="xprod-summary-list">
              ...
              <div className="xprod-sline"><span data-icon="▦">Total Area</span><strong>{standOnly ? "—" : `${((sideOpt.w * sideOpt.h) / CM2_PER_SQFT).toFixed(2)} sq.ft.`}</strong></div>
              <div className="xprod-sline is-wide"><span data-icon="✎">Material Finishing</span><strong>{standOnly ? "—" : sideOpt.finish}</strong></div>
            ...
            <div className="xprod-order" style={{ border: 0, background: "transparent", boxShadow: "none", margin: 0, padding: 0 }}>
              ...
              {added && (
                <div className="xprod-added">
                  <span>✓ Aluminium Aboard Stand 100cm x 200cm added to cart — {money(agents[0].price)}</span>

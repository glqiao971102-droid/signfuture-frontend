...
const PRODUCT_NAME = "Air Dancer";
const PRODUCT_HREF = "/catalog/air-dancer";
...
const SIZE = [
  { label: '42" x 144"', w: 1066.8, h: 3657.6 },
];
...
const PRICE: Record<string, number[]> = {
  "Printing with Stand": [1230.9, 615.45, 615.45, 615.45],
  "Printing Only": [320.1, 160.05, 160.05, 160.05],
  "Stand Only": [910.8, 455.4, 455.4, 455.4],
};
...
export default function AirDancerProduct() {
  const { user } = useAuth();
  const { add } = useCart();
  ...
  const addToCart = async () => {
    ...
    add({ artworks, label: PRODUCT_NAME, href: PRODUCT_HREF, price: total, image: "/products/air-dancer-hero.png", meta: `Finishing: ${finishing} · Material: ${standOnly ? "Stand only" : MATERIAL} · Size: ${size} · Qty: ${qty} · ${collectOpt.label}` });
    setAdded(true);
  };
  ...
  return (
    <div className="xprod">
      ...
      <section className="xprod-hero">
        ...
        <img
          src="/products/air-dancer-hero.png"
